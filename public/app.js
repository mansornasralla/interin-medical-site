...
function shell(view){document.body.className='appPage';const adminBtn=(me&&me.role==='admin')?'<button onclick="users()">إدارة المستخدمين</button>':'';$('#app').innerHTML=`<div class="wrap"><div class="top"><div>${logo()}<h1>الملف الطبي للزوار</h1><b>فريق انترين الطبي — M-Nexus</b></div><div class="nav"><button onclick="home()">الرئيسية</button><button onclick="archive()">الأرشيف</button>${adminBtn}<button onclick="localStorage.clear();login()">خروج</button></div></div><div id="view">${view}</div></div>`}
...
async function users(){
  if(!me||me.role!=='admin'){home();return}
  let arr=await api('/api/users');
  shell(`<h1>إدارة المستخدمين</h1><div class="card"><div class="grid" style="grid-template-columns:repeat(5,1fr)"><input id="un" placeholder="الاسم"><input id="ue" placeholder="البريد"><input id="up" placeholder="كلمة المرور"><select id="ur">${roles.map(r=>`<option>${r}</option>`).join('')}</select><button class="btn" onclick="addUser()">إضافة</button></div><div id="userMsg"></div>${arr.map(u=>`<div class="patientRow"><b>${u.name}</b><span>${u.email}</span><span>${u.role} - ${u.specialty}</span>${u.id===me.id?'<span>حسابك الحالي</span>':`<button class="btn danger" onclick="delUser(${u.id})">حذف</button>`}</div>`).join('')}</div>`)
}
async function addUser(){
  try{
    await api('/api/users',{method:'POST',body:JSON.stringify({name:$('#un').value,email:$('#ue').value,password:$('#up').value,role:$('#ur').value,specialty:$('#ur').value})});
    users();
  }catch(e){$('#userMsg').innerHTML=`<div class="danger btn" style="margin:12px 0">${e.message}</div>`}
}
async function delUser(id){
  if(confirm('هل تريد حذف هذا المستخدم؟')){
    try{await api('/api/users/'+id,{method:'DELETE'});users()}
